...
var style = {
  ...
  create: function create(_ref) {
    ...
    var colors = [];
    Object.keys(legacy__default["default"].color.collection).forEach(function (color) {
      if (options.colors === undefined || options.colors.length === 0 || options.colors.indexOf(color) !== -1) {
        var colorCollection = legacy__default["default"].color.collection[color];
        colors.push(colorCollection[options.colorLevel || defaults.colorLevel]);
      }
    });
    var color = prng.pick(colors); // prettier-ignore
